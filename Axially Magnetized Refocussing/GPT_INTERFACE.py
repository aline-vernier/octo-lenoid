...
def gpt_run(MAGNET_FILE, OUTSF7_LOC, psf_w_dir, partNum, bmap_offset, GPT_ROOT, scanParam,
    ...
    if explore == 0:
        ...
        l_map_array = np.linspace(0.045, 0.046, 10)
    ###################################################################
    ...
    text = \
        ...
        + "# Start initial beam \n " \
        + "npart = " + str(npart) + ";\n\n" \
        + "setparticles(\"beam\",npart,me,qe,npart*qe) ;\n\n" \
        + "# transverse \n" \
        ...
        + "rmax(\"wcs\",\"z\", " + Lpinhole  + " + 20e-3, 5e-3, 40e-3);		# remove particles with r>2.5mm \n" \
